...
const BookPage = () => {
    ...
    useEffect(() => {
        const book = bookList.filter((a) => a.id === parseInt(id))
        setSelectedBook(book[0])
    }, [id])

    useEffect(() => {
        const books = bookList.filter((b) => b.author === selectedBook.author && b.name !== selectedBook.name)
    ...
    <section className='container'>
        <div className="book-page">
            <div className="book-info">
                <img src={selectedBook.img} alt={selectedBook.name} />
                <div className="book-info-text">
                    <p>Published: {selectedBook.publishedDate}</p>
                    <p>Format: {selectedBook.format}</p>
            ...
            <div className="book-detail">
                ...
                <NewLineText text={selectedBook.text}/>
                <a href={selectedBook.link} target='_blank'><button>Order Now From Amazon</button></a>
            </div>
        </div>
        <div className="books-other">
            <h3>Others Books From {selectedBook.author}</h3>
            <div className="books-other-container">
                {filteredBookList.length == 0 && (
                    <p>No Other Books Published</p>
                )}
                {filteredBookList.map((b) => (
                  <BookCard name={b.name} img={b.img} id={b.id} />
                ))}
            </div>
        </div>
    </section>
